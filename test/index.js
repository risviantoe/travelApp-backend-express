...
describe('API ENDPOINT TEST', () => {
	...
	it('GET Detail Page', (done) => {
		chai.request(app)
			.get('/api/v1/member/detail/63412706259a4aaec326470c')
			.end((err, res) => {
				expect(err).to.be.null;
				expect(res).to.have.status(200);
				expect(res.body).to.be.an('object');
	...
	it('POST Booking Page', (done) => {
		const image = __dirname + '/buktipembayaran.jpg';
		const dataSample = {
			image,
			itemId: '63412706259a4aaec326470c',
			duration: 2,
			bookingStartDate: '9-10-2022',
			bookingEndDate: '11-10-2022',
			firstName: 'Edwin',
			lastName: 'Risvianto',
			email: 'erisvianto@gmail.com',
			phoneNumber: '085155022840',
			accountHolder: 'Edwin Risvianto',
			bankFrom: 'BCA',
		};
		chai.request(app)
